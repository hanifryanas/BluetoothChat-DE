...
public class MessageSqlite extends SQLiteOpenHelper{
    ...
    private static final String DATABASE_NAME = "MessageDE",
        TABLE_Messages = "Messages", Me_Tag="Me", Msg_Tag="Msg", Recu_Tag="Recu",
        Adresse_Tag="Adresse", CREATE_Messages_TABLE = "CREATE TABLE "+TABLE_Messages+"("+
            Recu_Tag+"INTEGER PRIMARY KEY, "+Msg_Tag+"TEXT, "+Me_Tag+"INTEGER, "+Adresse_Tag+
            "TEXT)";
    private Context c;
    ...
    public void updateMessage(cMessage Message) {
        ...
        values.put(Me_Tag, Message.getMe());
        values.put(Adresse_Tag, Message.getAdresse());
        db.update(TABLE_Messages, values, Recu_Tag + " = ? " + Adresse_Tag + " = ? "
                , new String[]{String.valueOf(date), Message.getMsg()});
        db.close();
    }
    public cMessage getMessage(long date, String adresse) {
        ...
        Cursor cursor = db.query(TABLE_Messages,
                ...
                Recu_Tag + " = ? and " + Adresse_Tag + " = ? ", new String[]
                        {String.valueOf(date), adresse},
                null, null, null, null);
        if(cursor!=null)
            cursor.moveToFirst();
        else
    ...
    public ArrayList<cMessage> getMessages(String adresse){
        ArrayList<cMessage> CaracterisedMessages = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(TABLE_Messages,
                new String[]{Recu_Tag,Msg_Tag,Me_Tag,Adresse_Tag},
        ...
        if (cursor!=null && cursor.moveToFirst())
            do{
                CaracterisedMessages.add(new cMessage(cursor.getLong(0),
                        cursor.getString(1),cursor.getInt(2),cursor.getString(3)));
            }while (cursor.moveToNext());
        db.close();
        return CaracterisedMessages;
    }
